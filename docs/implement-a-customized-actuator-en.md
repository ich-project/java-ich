...
Having a tailored actuator is key to building a java-ich-based customized public chain. This article illustrates how to develop a java-ich-based `SumActuator`.
...
Currently, contracts supported by java-ich are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
syntax = "proto3";
package protocol;
option java_package = "org.ich.core.contract"; //Specify the name of the package that generated the Java file
...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Ich.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Ich.proto` file:
...
```
At last, recompile the modified proto files. Compiling the java-ich project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, java-ich uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
# or build via protoc
protoc -I=src/main/protos -I=src/main/protos/core --java_out=src/main/java  Ich.proto
...
For now, the default Actuator supported by java-ich is located in `org.ich.core.actuator`. Creating `SumActuator` under this directory:
...
INFO [net] IchNetService start successfully.
