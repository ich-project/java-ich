...
import org.ich.core.db2.common.DB;
import org.ich.core.db2.common.IRevokingDB;
import org.ich.core.db2.common.LevelDB;
import org.ich.core.db2.common.RocksDB;
import org.ich.core.db2.core.Chainbase;
import org.ich.core.db2.core.IIchChainBase;
import org.ich.core.db2.core.RevokingDBWithCachingOldValue;
...
@Slf4j(topic = "DB")
public abstract class IchStoreWithRevoking<T extends ProtoCapsule> implements IIchChainBase<T> {
  ...
  protected IchStoreWithRevoking(String dbName) {
  ...
  protected IchStoreWithRevoking(DB<byte[], byte[]> db) {
    int dbVersion = CommonParameter.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected IchStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
        (AbstractRevokingStore) revokingDatabase);
  }

  // only for test
  protected IchStoreWithRevoking(String dbName, Options options,
